...
class Galeri extends CI_Controller{
	...
	function simpan_galeri(){
				$config['upload_path'] = './img_database/gallery/'; //path folder
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
	                        ...
	                        $config['source_image']='./img_database/gallery/'.$gbr['file_name'];
	                        $config['create_thumb']= FALSE;
	                        $config['maintain_ratio']= FALSE;
	                        $config['quality']= '60%';
	                        $config['width']= 500;
	                        $config['height']= 400;
	                        $config['new_image']= './img_database/gallery/'.$gbr['file_name'];
	...
	function update_galeri(){
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
							...
							$path='./img_database/gallery/'.$images;
							unlink($path);
							$kode=$this->session->userdata('idadmin');
							$user=$this->m_pengguna->get_pengguna_login($kode);
							$p=$user->row_array();
	...
	function hapus_galeri(){
		...
		$path='./img_database/gallery/'.$gambar;
		unlink($path);
