...
class Pengguna extends CI_Controller{
	...
	function simpan_pengguna(){
	            $config['upload_path'] = './img_database/pengguna/'; //path folder
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
	                        ...
	                        $config['source_image']='./img_database/pengguna/'.$gbr['file_name'];
	                        $config['create_thumb']= FALSE;
	                        $config['maintain_ratio']= FALSE;
	                        $config['quality']= '60%';
	                        $config['width']= 300;
	                        $config['height']= 300;
	                        $config['new_image']= './img_database/pengguna/'.$gbr['file_name'];
	...
	function hapus_pengguna(){
		...
		$q=$data->row_array();
		$p=$q['pengguna_photo'];
		$path=base_url().'img_database/pengguna/'.$p;
		delete_files($path);
